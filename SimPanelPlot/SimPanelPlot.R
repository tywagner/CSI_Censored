...
mean15 <- readRDS('mean15percent.rds')
quant15 <-readRDS('quant15percent.rds')

mean30 <- readRDS('mean30percent.rds')
quant30 <-readRDS('quant30percent.rds')

mean50 <- readRDS('mean50percent.rds')
quant50 <-readRDS('quant50percent.rds')
...
## 15%
plot(c(-0.6, 0.7), c(1-const,nlake + const), 
...
legend(0.5, 5, c("SD", "Slope", "Intercept"), pch = c(16,16,16), lty = c(3,2,1), 
...
segments(x0=quant30[,1,][1,], x1=quant30[,1,][2,],
         y0=1:nlake, y1=1:nlake, col='black',lwd=1)
## mean diff for ints
points(mean30[1,], 1:nlake, col='black',cex=1, pch=16)

# 95% CIs for slopes
segments(x0=quant30[,2,][1,], x1=quant30[,2,][2,],
         y0=1:nlake+const, y1=1:nlake+const, col='black',lwd=1, lty=2)
## mean diff for slopes
points(mean30[2,], 1:nlake+const, col='blue',cex=1, pch=16)

# 95% CIs for sd
segments(x0=quant30[,3,][1,], x1=quant30[,3,][2,],
         y0=1:nlake+const2, y1=1:nlake+const2, col='black',lwd=1, lty=3)
## mean diff for sd
points(mean30[3,], 1:nlake+const2, col='red',cex=1, pch=16)
...
segments(x0=quant50[,1,][1,], x1=quant50[,1,][2,],
         y0=1:nlake, y1=1:nlake, col='black',lwd=1)
## mean diff for ints
points(mean50[1,], 1:nlake, col='black',cex=1, pch=16)

# 95% CIs for slopes
segments(x0=quant50[,2,][1,], x1=quant50[,2,][2,],
         y0=1:nlake+const, y1=1:nlake+const, col='black',lwd=1, lty=2)
## mean diff for slopes
points(mean50[2,], 1:nlake+const, col='blue',cex=1, pch=16)

# 95% CIs for sd
segments(x0=quant50[,3,][1,], x1=quant50[,3,][2,],
         y0=1:nlake+const2, y1=1:nlake+const2, col='black',lwd=1, lty=3)
## mean diff for sd
points(mean50[3,], 1:nlake+const2, col='red',cex=1, pch=16)
