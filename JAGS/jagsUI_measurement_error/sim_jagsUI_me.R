...
# Proportion to have below RL but above DL
rlProp <- 0.50
...
dat$rlLimitVec <- as.numeric(rep(quantile(log.yiC,rlProp ),n))

dat$isNotCensored <- ( dat$log.yiC > dat$censorLimitVec ) # Must tell JAGS which observations are ABOVE Censoring limit

# Add an indicator for values that are above the censor limit, but below a hypothetical reporting limit
# RLCode = 1 if value is > DL, but below RL
dat$RLCode <- ifelse(dat$log.yiC < quantile(dat$log.yiC,cenProp ),0 ,ifelse(dat$log.yiC < quantile(dat$log.yiC, rlProp),1,0))

# Set Censored obs to NA
dat$log.yiC[!dat$isNotCensored] <- NA
...
cat("
    ...
    sigma.E[i] <- sigma + sigma.me * w[i] # Add additional measurement error for observations DL < yi < RL
    tau.E[i] <- pow(sigma.E[i],-2)
...
# Constraint on measurement error prior
# sigma.me needs to be less than RL-DL/4

me.prior <- (unique(dat$rlLimitVec) -unique(dat$censorLimitVec))/4

# JAGS dinterval needs 0,1 not T,F for isNotCensored
# Load data
data <- list(y = dat$log.yiC, x = dat$tp, n = n, 
             censorLimitVec = dat$censorLimitVec,
             isNotCensored = as.numeric(dat$isNotCensored),
             me.prior = me.prior,w = dat$RLCode )

# as.numeric(isNotCensored) = 1 if not censored and 0 if censored

# Initial values
yInit <- rep( NA , nrow(dat) )
yInit[!dat$isNotCensored] <- dat$censorLimitVec[!dat$isNotCensored]*0.5
inits <- function (){
...
#"sigma.E"
parameters <- c("a","b","sigma","sigma.me")
...
outME <- jags(data = data, inits = inits, parameters.to.save = parameters, 
...
# Censored data model with MEASUREMENT ERROR ###########################
############## BUGS MODEL #######################
...
# #####################################################
res <- 6
name_figure <- "SimDiff5PerJAGS.jpg"
