...
dat$ysub0 <- ifelse(is.na(dat$log.yiC), log(0.01), dat$log.yiC)
...
# NUmber of parameters to compare (int, slope, sigma)
# Hold MCMC for intercept
outDiff[i,1,1] <- out$mean$a 
# Hold MCMC for slope
outDiff[i,2,1] <- out$mean$b 
# Hold MCMC for sigma
outDiff[i,3,1] <- out$mean$sigma 

outDiff[i,1,2] <- out0.5$mean$a
# Hold MCMC for slope
outDiff[i,2,2] <- out0.5$mean$b 
# Hold MCMC for sigma
outDiff[i,3,2] <- out0.5$mean$sigma 

outDiff[i,1,3] <- outDL$mean$a
# Hold MCMC for slope
outDiff[i,2,3] <- outDL$mean$b 
# Hold MCMC for sigma
outDiff[i,3,3] <- outDL$mean$sigma 

outDiff[i,1,4] <- out0$mean$a 
# Hold MCMC for slope
outDiff[i,2,4] <- out0$mean$b 
# Hold MCMC for sigma
outDiff[i,3,4] <- out0$mean$sigma 

outDiff[i,1,5] <- outDrop$mean$a 
# Hold MCMC for slope
outDiff[i,2,5] <- outDrop$mean$b 
# Hold MCMC for sigma
outDiff[i,3,5] <- outDrop$mean$sigma 
...
legend(-0.95, 1.9, c('Truth','Censored','Set to 0.5*DL','Set to DL',"Omit"), lty=c(1,1,1,1,1,1), 
       lwd=c(5,5,5,5,5,5), col=c(plotcol[1],plotcol[2],plotcol[4],plotcol[5],plotcol[6]),
       text.col=c('black','black','black','black','black') ,x.intersp = 0.5)
